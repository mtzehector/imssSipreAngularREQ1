...
@Component({
  selector: 'app-carta-cap-informe',
  templateUrl: './carta.cap.informe.component.html',
})
export class CartaCapInformeComponent extends BaseComponent implements OnInit {
...
//    this.nuevo = new Prestamo();
	...
	this.cartaInstruccion.prestamo.tipoCreditoEnum = this.model.cartaInstruccion.prestamo.tipoCredito===1?TipoCredito.NUEVO:(this.model.cartaInstruccion.prestamo.tipoCredito===2?TipoCredito.RENOVACION:TipoCredito.COMPRA_CARTERA);
//    this.nuevo.tipoCreditoEnum = TipoCredito.NUEVO;
  ...
  obtenerValor(primerDescuentoResponse: FechaPrimerDescuento){
    ...
    navegarTerminosCondiciones(){
      this.router.navigate(['/promotor/terminoCondicionesCapacidad', {}]);
    }
}
